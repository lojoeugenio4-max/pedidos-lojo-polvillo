...
export default function PedidoDetallePage() {
  ...
  async function cargarPedido() {
    ...
    const { data: lineasData, error: lineasError } = await supabase
      .from("lineas_pedido")
      .select("id, pedido_id, codigo_articulo, nombre_articulo, cajas, unidades")
      .eq("pedido_id", id)
        ...
        {cargando ? (
          ...
          <section className="bg-white rounded-2xl p-6 shadow print:shadow-none print:rounded-none print:p-0">
            <header className="border-b pb-4 mb-4">
              <h1 className="text-3xl font-bold">Pedido para preparar</h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-4 text-sm">
                ...
                <p>
                  <strong>Estado:</strong> {pedido?.impreso ? "Impreso" : "Recibido"}
                </p>
              </div>
            </header>
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b-2 border-black">
                  ...
                  <th className="text-left py-2 pl-2 w-40">Kilo__________</th>
                </tr>
              </thead>
            ...
            {lineas.length === 0 && (
              <p className="text-slate-500 mt-4">Este pedido no tiene líneas.</p>
            )}
